...
# ///
"""Linux system-level install components (ADR-0003, kept through ADR-0013).

chezmoi + packages.py own the user-level tooling and dotfiles. What remains here
is the *system-level* software that needs root — Docker's daemon, CUDA, the
NVIDIA driver, LLVM's update-alternatives, apt system packages — plus Homebrew
itself on macOS. These are opt-in and run by ``scripts/setup.py`` after
``chezmoi apply`` has placed the files.
...
from managers import Deb, PackageManager, Script  # noqa: E402,F401
...
class Homebrew(OptionalComponent):
    ...
    def install(self, ctx):
        # User-level CLI tools come from packages.toml (via brew); this installs Homebrew *itself*
...
def install_codegraph(ctx):
    ...
    component: always run by ``setup_claude`` (scripts/setup.py), gated only by
...
def main():
    ...
    print("Available system components (run via: scripts/setup.py --system <list>)")
    print("=" * 60)
