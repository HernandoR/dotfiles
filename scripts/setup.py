...
def write_deferred_setup(ctx, agent_ids):
    ...
    lines = [
        ...
        *[f'  # smithery mcp add "{s}" --client claude || true' for s in smithery_servers],
        ...
        '  claude mcp add --transport http --scope user nowledge-mem "${_nmem_url}/mcp" \\',
        ...
        '    --header "X-Nowledge-Tool-Schema-Profile: slim" \\',
        '    && echo "nmem: nowledge-mem -> ${_nmem_url}/mcp" || echo "nmem: claude mcp add failed"',
        "else",
        ...
        r'  echo "post-login setup: kept at ${BASH_SOURCE[0]} — rerun once NMEM_API_KEY is set"',
        "else",
        '  rm -f "${BASH_SOURCE[0]}"',
        "fi",
    ]
