...
- **③ preference — not unified, and for pi seeded rather than owned.** Model,
  ...
  be a chezmoi-managed file — a repo-owned copy is excluded by construction, not
...
- **pi replaced omp in the third slot (2026-08-28; ADR-0012), for
  ...
  on ``PATH (env.zsh)``. The price is that every capability omp had natively is
...
# --- the shared root ---------------------------------------------------------
# ~/.agents is an env link (home/.chezmoidata/envlinks.toml, scripts/env_links.py) so everything below
...
#
#   1. the binary lands at ~/.local/bin/pi, which IS on PATH (env.zsh) — the
...
# $XDG_CONFIG_HOME/pi, else ~/.pi — so `home/.chezmoidata/envlinks.toml` setting
...
def repair_pi_npm_lockfile(ctx):
    ...
    **symlinked** path to ``npm install --prefix``, from whatever directory the
    ...
    the same variable) and needs a chezmoi apply, so it is a separate change.
    """
    try:
        lock = json.loads(PI_NPM_LOCKFILE.read_text())
    except (OSError, ValueError):
...
"""
    ...
    def install(self, ctx):
        ...
        # the setup phase (the old curl|bash call was non-fatal too).
    ...
    def plan(self, ctx, add):
        if shutil.which(self.binary):
            ...
            add("install", "pi via npm -g --prefix {} --ignore-scripts ({}), node from "
                           "mise — the prefix puts it on PATH (env.zsh), which mise "
...
def main():
    ...
    print("Agents (select with: scripts/setup.py --agents <list>)")
    print("=" * 66)
